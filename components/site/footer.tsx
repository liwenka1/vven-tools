...
const Footer = () => {
  ...
  return (
    <footer className="border-t border-gray-200 bg-white transition-colors duration-300 dark:border-zinc-800 dark:bg-zinc-900">
      <div className="container mx-auto px-6 py-12">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-500">数据本地处理，隐私安全保障</p>
          </div>

          {/* Links Sections */}
          {footerLinks.map((section) => (
            <div key={section.title}>
              <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">{section.title}</h3>
              <ul className="space-y-3">
                {section.links.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="text-sm text-gray-600 transition-colors duration-300 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400"
          ...
          <div className="flex flex-col items-center justify-between space-y-4 md:flex-row md:space-y-0">
            ...
            <div className="flex items-center space-x-6">
              <Link
                ...
                className="flex items-center space-x-2 text-sm text-gray-600 transition-colors duration-300 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400"
              >
              ...
              <div className="text-sm text-gray-500 dark:text-gray-500">v2.0.0</div>
            </div>
          </div>
        </div>
      </div>
